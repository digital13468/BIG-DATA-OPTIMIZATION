...
def gradient_descent(A_test, b_test, color, fig_error, rule, A, b, obj_list, x_init, x_list, max_iter=10000, alpha = 0, s = 0, beta = 0, sigma = 0):
    converged = False
    iter = 0
    x = x_init[:]
    x_list.append(x)
    ...
    while not converged:
        ...
        if iter in iter_list:
            ...
            #tstart = tend
    plot_error(color, fig_error, rule, time_diff_list, training_error_list, testing_error_list)
    return x

def init(A_test, b_test, rule, color, fig_error, A, x_init, b, max_iter, fig_objective, fig_line, alpha = 0, s = 0, beta = 0, sigma = 0):
    obj_list = []
    x_list = []
    x_converge = gradient_descent(A_test, b_test, color, fig_error, rule, A, b, obj_list, x_init, x_list, max_iter, alpha, s, beta, sigma)

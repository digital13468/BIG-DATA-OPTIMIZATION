...
def gradient_descent(rule, A, b, obj_list, x_init, x_list, max_iter=10000, alpha = 0, s = 0, beta = 0, sigma = 0):
    converged = False
    iter = 0
    x = x_init[:]
    x_list.append(x)
...
def init(rule, A, x_init, b, max_iter, fig_objective, fig_line, alpha = 0, s = 0, beta = 0, sigma = 0):
    obj_list = []
    x_list = []
    x_converge = gradient_descent(rule, A, b, obj_list, x_init, x_list, max_iter, alpha, s, beta, sigma)
...
def stepsize_rules(rule, A, x, b, max_iter, fig_objective, fig_line, lambda_max, A_test, b_test, s = 0, beta = 0, sigma = 0):
    ts = time.time()
    if rule == 'constant':
        ...
        #print datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        x_list, obj_list = init(rule, A, x, b, max_iter, fig_objective, fig_line, 0, s, beta, sigma)
        error_count(x_list[-1], A, b, 'training')
        error_count(x_list[-1], A_test, b_test, 'testing')
        #print x_list[-1]
        print obj_list[-1], len(obj_list)
...
def running_algorithm(A, b, max_iter, x, fig_objective, fig_line, A_test, b_test):
    #classifier_matrix = []  
    ...
    stepsize_rules('constant', A, x, b, max_iter, fig_objective, fig_line, 1.0/sort_w[len(sort_w)-1], A_test, b_test)
...
def data_set_problem(data, max_iter, x_init, test_data, data_type):
    visual_classifier = []
    A_digit = {}
    test_data_digit = {}
    #print test_data
    ...
    for i in range(2, 3):
        ...
        running_algorithm(A, b, max_iter, x_init, fig_objective, fig_line, A_test, b_test)
        if data_type == 'featured':
            PBCGD(len(x_init), A, b, A_test, b_test, max_iter, x_init, fig_objective, fig_line)
            #print x_init
            CBCGD(1, A, b, A_test, b_test, max_iter, x_init, fig_objective, fig_line)
            RBCGD(2, A, b, A_test, b_test, max_iter, x_init, fig_objective, fig_line)
        else:
        ...
        logistic_regression(A, b, x_init, A_test, b_test, max_iter, i, fig_line, 'Evolution of the objective function for digit-'+str(i)+' on '+data_type+' data')
        PLA_problem_setup(A, b, x_init, max_iter, i, fig_line, 'Evolution of the objective function for digit-'+str(i)+' on '+data_type+' data', A_test, b_test)
...
if __name__ == '__main__':
    ...
    data_set_problem(data, max_iter, x_init, test_data, 'featured')
    print 
